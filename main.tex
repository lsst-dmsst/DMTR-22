...
\begin{enumerate}
    ...
    \begin{itemize}
      \item Description : we test how  well does a coverage image reflect the queried region for an extended source (a galaxy) at ra,dec =  40.433\degree,  0.449\degree
      \item Input : perform cone query against the Deep Source table, using coordinates of ra,dec = 23h30m57.31s, +1d1m13.8s  ( or 352.73878\degree ,  1.02049\degree ), with search radii of 2\arcsec, 10\arcsec, 100\arcsec, or 1000\arcsec. Each time, compare the coverage to the SDSS DR13 Sky Server \url{http://skyserver.sdss.org/dr13/en/tools/chart/navi.aspx}. 
...
As a concrete example, we execute a cone query against the Deep Source table,  expecting to find  a point source: a star at  We find that the queried region only sometimes has the postage stamp miniature aligned with the query region. 

We also consider a galaxy located at ra,dec =  40.433\degree,  0.449\degree. A 10\arcsec cone query only sometimes is centered on the galaxy. 
